...
render_hero(
    ...
    "Que revelo el analisis exploratorio antes de modelar",
    "El EDA se uso para entender la estructura del dataset, el comportamiento del target y las senales mas utiles para diferenciar pobreza y no pobreza.",
)

render_section_intro(
    "El EDA no fue un tramite",
    "Esta etapa permitio ordenar variables, detectar posibles redundancias y justificar por que ciertas dimensiones del hogar debian entrar al modelado.",
)
...
with col_left:
    ...
    if df.empty:
        st.warning("No se encontro el dataset procesado para construir esta vista.")
    else:
        st.altair_chart(target_distribution_chart(df), width="stretch")
with col_right:
    st.markdown("### Lectura del target")
    if target_summary.empty:
        render_story_card("Target", "No se encontro el resumen del target en reports/eda.")
    else:
        for row in target_summary.itertuples():
            label = "No pobre" if int(row.target) == 0 else "Pobre"
            render_story_card(label, f"Hogares: {row.n} | Participacion: {row.pct:.2f}%")

st.markdown("### Variables numericas y categoricas")
...
with meta_cols[0]:
    render_story_card("Variables numericas", str(len(numeric_cols)))
with meta_cols[1]:
    render_story_card("Variables categoricas", str(len(categorical_cols)))
with meta_cols[2]:
    render_story_card("Insight", "Se observan senales en vivienda, servicios, salud, composicion e ingresos.")

if not df.empty:
    st.markdown("### Exploracion visual")
    ...
    with col1:
        selected_numeric = st.selectbox("Variable numerica", num_options, index=0 if num_options else None)
        if selected_numeric:
            st.altair_chart(numeric_distribution_chart(df, selected_numeric), width="stretch")
    with col2:
        selected_categorical = st.selectbox("Variable categorica", cat_options, index=0 if cat_options else None)
...
if numeric_by_target.empty and categorical_by_target.empty:
    ...
    render_story_card(
        "Interpretacion",
        "El EDA mostro que la pobreza no depende de una sola variable. La señal aparece combinando condiciones monetarias, de vivienda, servicios y composicion del hogar.",
    )
    if hallazgos:
