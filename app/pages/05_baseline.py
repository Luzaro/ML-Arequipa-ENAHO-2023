...
render_hero(
    ...
    "El baseline permite entender familias de modelos, detectar señales útiles y justificar por qué el tuning era necesario en un problema con clases desbalanceadas.",
)

if baseline.empty:
    st.warning("No se encontró el resumen baseline en reports/scenario_b_modeling.")
else:
    ...
    render_section_intro(
        "Por qué importó esta etapa",
        "El baseline no buscaba el modelo final. Buscaba mostrar desde dónde partíamos y qué tipo de modelo capturaba mejor la señal antes de tratar el desbalance.",
    )
    ...
    metric_map = {"accuracy": "Accuracy", "precision_pobre": "Precisión", "recall_pobre": "Recall", "f1_pobre": "F1"}

    st.markdown("### Comparación baseline")
