...
def write_checklist(path: Path, features_df: pd.DataFrame, discarded_df: pd.DataFrame) -> None:
    text = "\n".join(
        [
            ...
            f"- NumÃ©ricas: {(features_df['tipo_feature'] == 'numerica').sum()}",
            f"- CategÃ³ricas: {(features_df['tipo_feature'] == 'categorica').sum()}",
            f"- Binarias: {(features_df['tipo_feature'] == 'binaria').sum()}",
            f"- Variables descartadas o con leakage: {discarded_df.shape[0]}",
            "- Revisar imputaciÃ³n en columnas con nulos antes de entrenar.",
            "- Revisar encoding para las categÃ³ricas de alta cardinalidad.",
            "- Mantener fuera del entrenamiento las variables monetarias directas descartadas por riesgo de fuga.",
            "- Confirmar tratamiento de ponderadores en evaluaciÃ³n y no solo en entrenamiento.",
            "- PrÃ³ximo paso: modelado supervisado.",
        ]
    )
